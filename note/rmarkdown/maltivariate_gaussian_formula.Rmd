...
　多次元ガウス分布(Maltivariate Gaussian Distribution)・多変量正規分布(Maltivariate Normal Distribution)の定義を確認します。\
\


## 定義式

　多次元ガウス分布は、1次元ガウス分布を$D$次元に拡張(一般化)した分布です。2種類のパラメータによる定義式を確認します。\
\
...
　または、$\boldsymbol{\Lambda}$を用いて、次の式でも定義されます。
...
　ここで、$\boldsymbol{\mu}$は平均ベクトル、$\boldsymbol{\Sigma}$は分散共分散行列、$\boldsymbol{\Lambda}$は精度行列、$\pi$は円周率です。また、$\mathbf{A}^{\top}$は行列$\mathbf{A}$の転置行列、$\mathbf{A}^{-1}$は逆行列、$|\mathbf{A}|$は行列式、ネイピア数$e$による指数関数$e^x = \exp(x)$、平方根$\sqrt{a} = a^{\frac{1}{2}}$です。\
...
\tag{2'}\\
   &= \left(
      ...
      \right\}
    = \mathcal{N}(x = x_1 | \mu = \mu_1, \lambda^{-1} = \lambda_1^{-1})
\end{align*}
...
　続いて、変数間に依存関係がない(独立である)場合のガウス分布を考えます。\
\

　まずは、分散共分散行列について確認します。\

　各変数が独立のとき全ての共分散が0になります。
...
　各変数$x_d$の分散$\sigma_d^2$を対角成分とする対角行列になります。\
...
　各変数$x_d$の分散$\sigma_d^2$の逆数$\frac{1}{\sigma_d^2} = \sigma_d^{-2}$を対角成分とする対角行列になります。\
\
