...
- 2022/10045：「定義式」・「正規化項の導出」・「統計量の導出」を修正
...
　ベータ分布は、ベルヌーイ分布と二項分布の共役事前分布であり、$\phi$はベルヌーイ分布・二項分布のパラメータ$\phi, 1 - \phi$の条件を満たします。\
\
...
　連続値をとる確率分布の最頻値(mode)は、$x$についての確率密度関数$p(x)$の微分$\frac{\partial p(x)}{\partial x}$が0となる$x$です。\
\
