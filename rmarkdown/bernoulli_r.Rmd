...
　失敗確率($x = 0$となる確率)$1 - \phi$と成功確率($x = 1$となる確率)$\phi$をベクトルに格納して`phi_v`とします。これは、定義式にそれぞれ代入すると、0乗は1になるので($a^0 = 1$なので)、$x = 0$のとき$\mu^0 (1 - \mu)^{1-0} = 1 - \mu$、$x = 1$のとき$\mu^1 (1 - \mu)^{1-1} = \mu$になることに対応しています。\
