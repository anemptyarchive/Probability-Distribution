...
　ベルヌーイ分布(Bernoulli Distribution)の確率と統計量を計算します。ベルヌーイ分布については「定義式」を参照してください。\
\
...
　失敗確率($x = 0$となる確率)$1 - \phi$と成功確率($x = 1$となる確率)$\phi$をベクトルに格納して`phi_v`とします。\
...
　ベルヌーイ分布(Bernoulli Distribution)のグラフを作成します。ベルヌーイ分布については「定義式」を参照してください。\
\
...
　$x$がとり得る値$x \in \{0, 1\}$を作成して、`x_vals`とします。\
...
　`x_n`に含まれる`0`の要素数は、`sum(x_n == 0)`で得られます。`x_n == 0`とすると、値が`0`の要素は`TRUE`、それ以外の要素は`FALSE`が返ってきます。`TRUE`は`1`、`FALSE`は`0`として計算できるので、`sum()`で`TRUE`の数をカウントします。\
　値が`1`の要素数も同様に求めて、データフレームに格納します。\
　データ数`N`から`0`または`1`の要素数を引いてももう一方の要素数を求められます。\
