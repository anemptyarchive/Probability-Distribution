...
　ポアソン分布の確率は、`dpois()`で計算できます。確率変数の引数`x`に`x_vals`、パラメータの引数`lambda`に`lambda_n`の値を指定します。\
...
　`x_vals`と`lambda_n`それぞれの要素の全ての組み合わせを`expand_grid()`で作成して、組み合わせごとに確率を計算します。\
　ガウス分布の確率密度は、`dnorm()`で計算できます。平均の引数`mean`に`mu`、標準偏差の引数`sd`に`lambda_n`の値の逆数の平方根を指定します。\
