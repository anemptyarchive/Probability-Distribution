...
# ベルヌーイ分布の確率を計算
prob_lt = [
    bernoulli.pmf(k=x_vec, p=phi_vals[i]) for i in range(frame_num)
]
...
# 動画を書出
anim.save(
    filename='../../figure/bernoulli/parameter/parameter_phi.mp4', 
...
def update(i):
    ...
    # 統計量を計算
    mean_x = phi                      # 期待値
    sd_x   = np.sqrt(phi * (1.0-phi)) # 標準偏差
    mode_x = 1 if phi >= 0.5 else 0   # 最頻値
    ...
    for i, coord_x in enumerate([mean_x-sd_x, mean_x+sd_x]):
        ax.axvline(
            x=coord_x, 
            color='black', linewidth=1.0, linestyle=':', 
            label=f'$\\sqrt{{V[x]}} = \\sqrt{{\\phi (1-\\phi)}} = {sd_x:.2f}$' if i == 0 else None, 
        zorder=11
        ) # 標準偏差の位置
    ax.hlines(
    ...
    ax.axvline(
        x=mode_x, 
        color='black', linewidth=1.0, linestyle='-.', 
        label=f'$mode[x] = 0\ (\phi \leq 0.5), 1\ (\phi \geq 0.5) = {mode_x:.2f}$', 
        zorder=11
    ) # 最頻値の位置
    ax.text(
        x=mean_x, y=0.0, 
...
# 動画を書出
anim.save(
    filename='../../figure/bernoulli/parameter/stats_phi.mp4', 
    progress_callback=lambda i, n: print(f'\rframe: {i+1} / {n}', end='', flush=True)
)
...
def update(i):
    ...
    # 統計量を計算
    mu    = phi                      # 期待値
    sigma = np.sqrt(phi * (1.0-phi)) # 標準偏差

    # モーメントを計算
    if phi > 0.0 and phi < 1.0: # パラメータの条件
        skew  = 1.0 - 2.0 * phi             # 歪度
        skew /= np.sqrt(phi * (1.0-phi))
...
# 動画を書出
anim.save(
    filename='../../figure/bernoulli/parameter/moment_phi.mp4', 
    progress_callback=lambda i, n: print(f'\rframe: {i+1} / {n}', end='', flush=True)
)
