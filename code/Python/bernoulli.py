...
# 対数をとった定義式により確率を計算
ln_prob = x * np.log(phi) + (1.0 - x) * np.log(1.0 - phi)
prob = np.exp(ln_prob)
print(prob, ln_prob)
...
ln_prob = bernoulli.logpmf(k=x, p=phi)
prob = np.exp(ln_prob)
print(prob, ln_prob)
...
ln_prob = binom.logpmf(k=x, n=1, p=phi)
prob = np.exp(ln_prob)
print(prob, ln_prob)
...
ln_prob = multinomial.logpmf(x=x_v, n=1, p=phi_v)
prob = np.exp(ln_prob)
print(prob, ln_prob)
